...
/**
 * Fetch profile list (kind 30000) by address. Address format: 30000:pubkey:d
 */
export async function fetchProfileListFromRelays(relayUrls, address) {
...
/**
 * Fetch forum posts (kind 11) for a community, optionally filtered by authors (profile list members).
 */
export async function fetchCommunityForumPosts(relayUrls, communityPubkey, authorPubkeys = []) {
	const filter = {
		kinds: [EVENT_KINDS.FORUM_POST],
		'#h': [communityPubkey],
		limit: 100
	};
	if (authorPubkeys.length > 0) filter.authors = authorPubkeys;
...
/**
 * Fetch kind 1 comments that reference a forum post (#e = eventId).
 */
export async function fetchForumPostComments(eventId, options = {}) {
	...
	const filter = { kinds: [1], '#e': [id], limit: 500 };
	const events = await fetchFromRelays(relays, filter, { timeout, signal });
...
export async function fetchZapsByEventIds(eventIds, options = {}) {
	const { timeout = 5000, signal, relays = DEFAULT_SOCIAL_RELAYS } = options;
