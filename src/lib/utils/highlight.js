 ...
 * CodeBlock's `html` prop.  Falls back to escaped plain text for unknown
 * languages so it's always safe to call unconditionally.
 *
...
export function highlightCode(code, lang) {
    ...
    try {
        ...
        // Auto-detect for unlabelled blocks (best-effort)
...
/** Escape HTML special chars — kept as a public util for other callers. */
...
/**
 * Highlight a pre-stringified JSON string (kept for DetailsTab backward compat).
 * Now delegates to highlightCode so the visual result is consistent everywhere.
 * @param {string} json
 * @returns {string}
 */
