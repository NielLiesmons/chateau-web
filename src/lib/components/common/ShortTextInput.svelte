    ...
    const itemsHtml = items
        ...
        if (type === "profile") {
          ...
          `;
            ...
            const e = item;
            const customImgHtml = e.source !== "unicode"
                ? `<span class="emoji-img-wrap"><img src="${escapeAttr(e.url)}" alt="" class="emoji-img emoji-img-loading" onload="this.classList.remove('emoji-img-loading')" onerror="this.parentNode.innerHTML='<span class=\\'emoji-img-error\\'>?</span>'" /></span>`
                : "";
            return `
            <button type="button" class="suggestion-item ${index === selected ? "selected" : ""}" data-index="${index}">
              ${e.source === "unicode" ? `<span class="emoji-unicode">${e.url}</span>` : customImgHtml}
...
<style>
  ...
  :global(.emoji-img-loading) {
    background: hsl(var(--white16));
    border-radius: 4px;
    color: transparent;
  }
  :global(.emoji-img-error) {
    font-size: 14px;
    color: hsl(var(--white33));
  }
