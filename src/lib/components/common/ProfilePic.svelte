<script>
	...
	/** @type {string|null|undefined} - Hex pubkey or npub for color generation (npub is decoded to hex for consistent color) */
	 ...
	 * @param {string|null|undefined} pubkeyVal - Hex pubkey or npub (decoded to hex for color)
	...
	function getProfileColor(pubkeyVal, nameVal) {
		if (pubkeyVal && pubkeyVal.trim()) {
			...
			// npub: decode to hex so hexToColor produces the profile's consistent color (like webapp)
			...
			if (/^[0-9a-fA-F]+$/.test(raw)) return hexToColor(raw);
		}
