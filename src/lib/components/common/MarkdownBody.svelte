 ...
 *   <MarkdownBody {tokens} />
 *
 * To add custom block rendering, add a new {:else if token.type === '...'} branch.
 * To add custom inline rendering, edit MarkdownInline.svelte.
 *
...
{#each tokens as token}
	...
	{:else if token.type === 'table'}
		<!-- Outer div provides the rounded corners (overflow:hidden clips the table) -->
...
<style>
	...
	/* Lists — Tailwind base resets list-style; re-assert here.
	   ol needs slightly more padding for the number glyph; matching that
	   value for ul keeps the left edges of markers visually aligned. */
	ul {
	...
	th, td {
		padding: 8px 12px;
		text-align: left;
		/* Cells hug their content but cap at a viewport-relative width so no
		   single column can stretch unreasonably on any screen size. */
