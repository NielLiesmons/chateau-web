<script lang="js">
	...
	let heroElement = $state(null);
	/** @type {HTMLButtonElement | null} */
	let exploreButton = $state(null);
	let scrollY = $state(0);
	...
	const emojiConfigs = [
		{ imageUrl: `/images/emoji/community.png`, x: -480, y: -190, size: 1.4, rotation: 12 },
		{ imageUrl: `/images/emoji/forum.png`, x: 460, y: -210, size: 1.5, rotation: -15 },
		{ imageUrl: `/images/emoji/chat.png`, x: -400, y: 155, size: 1.5, rotation: 20 },
		{ imageUrl: `/images/emoji/group.png`, x: 420, y: 160, size: 1.4, rotation: -18 },
	...
	function calculateParallaxSpeed(size) {
		...
		return minSpeed + ((normalizedSize - minSize) / (maxSize - minSize)) * (maxSpeed - minSpeed);
	}
	...
	function calculateOpacity(size) {
		...
		const normalizedSize = Math.max(minSize, Math.min(maxSize, size));
		return minOpacity + ((normalizedSize - minSize) / (maxSize - minSize)) * (maxOpacity - minOpacity);
	}
...
>
	...
	<div class="absolute left-0 right-0 top-0 h-[490px] sm:h-full pointer-events-none">
	...
	<div class="relative z-10 text-center px-4 mt-8 sm:mt-12 lg:mt-0">
		<h1 class="text-5xl sm:text-6xl lg:text-7xl xl:text-8xl font-bold mb-4 hero-title">
			<span style="background: var(--gradient-gray); -webkit-background-clip: text; background-clip: text; color: transparent;">
				Community.
			</span>
			<br />
			<span style="background: var(--gradient-blurple-light); -webkit-background-clip: text; background-clip: text; color: transparent;">
				Collaboration.
			</span>
		</h1>
