<script lang="js">
	...
	const currentNpub = $derived(currentPubkey ? (() => { try { return nip19.npubEncode(currentPubkey); } catch { return ''; } })() : '');
...
>
	<nav class="container mx-auto h-full px-4 sm:px-6 md:px-8">
		<div class="flex items-center justify-between gap-6 h-full">
			...
			<!-- CTA: when profile added show Explore + avatar; else Get Started -->
			{#if currentPubkey}
				<div class="flex items-center gap-3">
					<a href="/communities" class="btn-primary-small h-10 px-4 flex items-center justify-center">
						<span class="sm:hidden">Communities</span>
						<span class="hidden sm:inline">Explore Communities</span>
					</a>
					<a href="/communities" class="profile-pic-link" aria-label="Go to communities">
						<ProfilePic
							pubkey={currentNpub}
							name={null}
							pictureUrl={null}
