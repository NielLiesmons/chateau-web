<script>
  ...
  /**
   * Forum Post card: top row = avatar column (pic + vertical line) and right column (profile info, title, content, labels)
   * sitting right next to the avatar. Each row in the right column has 12px left/right padding; the labels row uses
   * 12px mask zones (fade) instead of padding so labels can scroll under the edges.
   */
  let {
  ...
  /** @param {number|string|null|undefined} ts - Unix seconds or date string */
...
<style>
  ...
  /* Labels: no padding; 12px mask zones on left and right (in place of the row padding above) so labels can scroll under a fade */
  .labels-mask-wrap {
    overflow: hidden;
  }
  ...
  .labels-scroll {
    ...
    padding: 0 0 0 12px;
    /* 12px left mask zone; 16px right so total right padding is 16px (card has 16px) */
