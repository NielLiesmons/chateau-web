...
import ChateauHeader from '$lib/components/landing/ChateauHeader.svelte';
...
// Chateau landing onboarding modals
...
// Communities explorer is full-screen two-column; no site header
let isCommunitiesPage = $derived(path.startsWith('/communities'));
...
<div class="min-h-screen relative bg-background">
	...
	<div class="relative z-10 flex flex-col min-h-screen">
		...
		{#if isReachKit}
			{@render children()}
		{:else}
			{#if !isDetailPage && !isCommunitiesPage}
				{#if isLandingPage}
					<ChateauHeader onGetStarted={openGetStartedModal} />
				{:else}
					<Header
			...
			<main class="flex-1 main-content" class:has-header={!isDetailPage && !isCommunitiesPage}>
				{@render children()}
			</main>

			{#if !isDetailPage && !isCommunitiesPage && path !== '/apps'}
				<Footer />
			{/if}
			...
			{#if false && dev && !isReachKit}
				<!-- Bust local cache button (commented out at app level) -->
